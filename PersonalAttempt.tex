...
If two complexity classes were identical, then any systematic method of measuring how problems enter those classes as resource bounds increase would necessarily yield identical behaviour.
In this framework, that would imply that $\Delta_P(p)$ and $\Delta_{NP}(p)$ coincide for all polynomial bounds. The intuition was therefore that a persistent discrepancy in these “rates of change” might suggest structural separation.
...
Therefore, while the framework is mathematically coherent, it fails to satisfy a central requirement of complexity-theoretic separation: invariance under polynomial-time reductions.
Since complexity classes are defined extensionally via reductions rather than cardinality, any approach dependent on counting arguments risks mischaracterising structural equivalence.
This realisation marked a turning point in the project, shifting emphasis from structural intuition to methodological constraint.
